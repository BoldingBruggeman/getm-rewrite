...
!  Local variables
   integer :: stat
!-----------------------------------------------------------------------
   call mm_s('dR',self%dR,self%domain%T%l,self%domain%T%u,def=0._real64,stat=stat)
   call mm_s('dZ',self%dZ,self%dR,def=0._real64,stat=stat)
   call mm_s('P',self%P,self%dR,def=0._real64,stat=stat)
         ...
         if (TG%mask(i,j) > 0) then
            ...
            do k=TG%kmax-1,1,-1
               self%P(i,j,k)=self%P(i,j,k+1)+0.5_real64*((buoy(i,j,k+1)+buoy(i,j,k))* &
                    (TG%zc(i,j,k+1)-TG%zc(i,j,k))-OneFifth*((self%dR(i,j,k+1)-self%dR(i,j,k))* &
                    (TG%zc(i,j,k+1)-TG%zc(i,j,k)-1./12.*(self%dZ(i,j,k+1)+self%dZ(i,j,k)))- &
                    (self%dZ(i,j,k+1)-self%dZ(i,j,k))*(buoy(i,j,k+1)-buoy(i,j,k)- &
                    1._real64/12._real64*(self%dR(i,j,k+1)+self%dR(i,j,k)))))
            end do
         end if
      end do
   end do
         ...
         do i=UG%imin,UG%imax
            if (UG%mask(i,j) > 0) then
               self%FC(i,j) = 0.5*((buoy(i+1,j,k)+buoy(i,j,k))*(TG%zc(i+1,j,k)-TG%zc(i,j,k)) &
                         -0.2_real64*( &
                          (self%dRx(i+1,j)-self%dRx(i,j))*(TG%zc(i+1,j,k)-TG%zc(i,j,k)-x*(self%dZx(i+1,j)+self%dZx(i,j))) &
                         -(self%dZx(i+1,j)-self%dZx(i,j))*( buoy(i+1,j,k)- buoy(i,j,k)-x*(self%dRx(i+1,j)+self%dRx(i,j)))))
               self%idpdx(i,j,k)=UG%hn(i,j,k)/UG%dx(i,j) &
                                 *(self%P(i+1,j,k)-self%P(i,j,k)+self%FC(i,j) &
                                 -(UG%sseo(i+1,j)-UG%sseo(i,j))*0.5_real64*(buoy(i+1,j,TG%kmax)+buoy(i,j,TG%kmax)))
#if 0
               self%idpdx(i,j,k)=AJ*dxm1*UG%hn(i,j,k) - UG%hn(i,j,k)*dxm1*(UG%sseo(i+1,j)-UG%sseo(i,j))*0.5_real64*(buoy(i+1,j,kmax)+buoy(i,j,kmax))
#endif
            end if
         end do
      end do
      ...
      do j=VG%jmin,VG%jmax
         do i=VG%imin,VG%imax
            if (VG%mask(i,j) > 0) then
               self%FC(i,j) = 0.5_real64*((buoy(i,j+1,k)+buoy(i,j,k))* &
                      (TG%zc(i,j+1,k)-TG%zc(i,j,k))-OneFifth*((self%dRx(i,j+1)-self%dRx(i,j))* &
                      (TG%zc(i,j+1,k)-TG%zc(i,j,k)- &
                      1._real64/12._real64*(self%dZx(i,j+1)+self%dZx(i,j)))-(self%dZx(i,j+1)-self%dZx(i,j))* &
                      (buoy(i,j+1,k)-buoy(i,j,k)-1._real64/12._real64*(self%dRx(i,j+1)+self%dRx(i,j)))))
               self%idpdy(i,j,k)=VG%hn(i,j,k)/VG%dy(i,j) &
                                 *(self%P(i,j+1,k)-self%P(i,j,k)+self%FC(i,j) &
                                 -(VG%sseo(i,j+1)-VG%sseo(i,j))*0.5_real64*(buoy(i,j+1,TG%kmax)+buoy(i,j,TG%kmax)))
            end if
         end do
      end do
      end associate VGrid
