...
def check_range(name, values, rtol=1e-12, atol=1e-12, target_value=None):
    ...
    print(
        "%s (mean %.6g, spread %.6g, relative spread %.6g)"
        % (result, mean, absrange, relrange)
    )
    if not valid:
...
def test(
    ...
    success = check_range(
        ...
        sim.momentum.U,
        target_value=None
        if apply_bottom_friction
        else ntime * timestep * tau_x / pygetm.RHO0,
    )
    success = (
        check_range(
            "V",
            ...
            target_value=None
            if apply_bottom_friction
            else ntime * timestep * tau_y / pygetm.RHO0,
        )
        and success
    )
...
if __name__ == "__main__":
    ...
    success = test(
        ...
        periodic_x=True,
        tau_x=0.01,
        apply_bottom_friction=args.apply_bottom_friction,
    )
    success = (
        test(
            "Periodic in x",
