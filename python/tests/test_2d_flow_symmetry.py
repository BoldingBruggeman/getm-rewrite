...
class Debug:
    def check_symmetry(
        ...
        if name is not None:
            print(
                "  Asymmetry in %s: %s" % (name, asym / ran),
...
def test(
    ...
    success = True
    success = (
        deb.check_symmetry(dp, name="surface pressure gradient", axis=symmetry_axis)
        and success
    )
    ...
    if meanz is not None:
        success = deb.compare("  Mean sea level: %s m" % (meanz,), meanz, 0.0)
