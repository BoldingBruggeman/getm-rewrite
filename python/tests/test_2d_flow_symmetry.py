...
def test(name: str, periodic_x: bool=False, periodic_y: bool=False, tau_x: float=0., tau_y: float=0., timestep: float=10., ntime: int=360, apply_bottom_friction: bool=False) -> bool:
    ...
    u_T = domain.T.map(sim.U_, domain.U)[2:-2, 2:-2] / domain.T.D
    v_T = domain.T.map(sim.V_, domain.V)[2:-2, 2:-2] / domain.T.D
    ...
    E_input = (E_input * numpy.ma.array(domain.T.area, mask=domain.T.mask==0)).sum()
