...
class Domain(_pygetm.Domain):
    ...
    def update_depth(self, _3d: bool=False):
        ...
        if _3d:
            # Update layer thicknesses (hn) on the T grid, using bathymetry H and new elevations zin (on the 3D timestep)
            # For vertical coordinates that only process the interior (no halos), suh as adaptive coordinates, we do a full halo exchange
            self.T.ho.all_values[...] = self.T.hn.all_values
            self.do_vertical()
            self.T.hn.update_halos()

            # Calculate the thicknesses at 1/2 a timestep earlier; these will be used as basis for U, V, X thicknesses,
            # and in turn for thicknesses on the advection grids
            self.h_T_half.all_values[...] = 0.5 * (self.T.ho.all_values + self.T.hn.all_values)

            # Update thicknesses on U grid and ensure they are valid throughout halos (needed to compute velocities, among others)
            self.U.ho.all_values[...] = self.U.hn.all_values
            self.h_T_half.interp(self.U.hn)
            self.U.hn.update_halos(parallel.Neighbor.RIGHT)

            # Update thicknesses on V grid and ensure they are valid throughout halos (needed to compute velocities, among others)
            self.V.ho.all_values[...] = self.V.hn.all_values
            self.h_T_half.interp(self.V.hn)
            self.V.hn.update_halos(parallel.Neighbor.TOP)

            # Update thicknesses on X grid. These will be valid in the interior strip of the halos only,
            # which is enough for their use on advection grids below.
