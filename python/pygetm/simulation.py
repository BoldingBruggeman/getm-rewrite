...
class Simulation(_pygetm.Simulation):
    ...
    _all_fortran_arrays = tuple(['_%s' % name for name in _pressure_arrays + _sealevel_arrays])
    __slots__ = _all_fortran_arrays + ('output_manager', 'input_manager', 'fabm', '_yearday', 'tracers', 'tracer_totals', 'logger', 'momentum', 'airsea', 'turbulence', 'density', 'buoy', 'temp', 'salt', 'pres', 'rad', 'par', 'par0', 'rho', 'sst', 'sss', 'NN', 'ustar_s', 'ustar_b', 'taub', 'z0s', 'z0b', 'fwf', '_cum_river_height_increase', '_start_time', '_profile', 'radiation', '_initialized_variables') + _time_arrays
    ...
    def __init__(self, dom: domain.Domain, runtype: int, advection_scheme: operators.AdvectionScheme=operators.AdvectionScheme.HSIMT, apply_bottom_friction: bool=True, fabm: Union[pygetm.fabm.FABM, bool, str, None]=None, gotm: Union[str, None]=None,
        ...
        super().__init__(dom, runtype, internal_pressure_method=internal_pressure_method)
        ...
        self.momentum = pygetm.momentum.Momentum(self.logger.getChild('momentum'), dom, runtype, apply_bottom_friction=apply_bottom_friction, Am=Am, advection_scheme=advection_scheme)

        self.domain.open_boundaries.z = self.wrap(self.domain.open_boundaries.z, b'zbdy', source=3)
        self.domain.open_boundaries.u = self.momentum.wrap(self.domain.open_boundaries.u, b'bdyu')
        self.domain.open_boundaries.v = self.momentum.wrap(self.domain.open_boundaries.v, b'bdyv')
    ...
    def advance(self):
        ...
        self.momentum.advance_depth_integrated(self.timestep, self.airsea.taux_U, self.airsea.tauy_V, self.dpdx, self.dpdy)
        ...
        self.advance_surface_elevation(self.timestep, self.momentum.U, self.momentum.V, self.fwf)
        ...
        if self.runtype > BAROTROPIC_2D and macro_active:
            ...
            self.momentum.advance(self.macrotimestep, self.split_factor, self.airsea.taux_Uo, self.airsea.tauy_Vo, self.dpdx, self.dpdy, self.idpdx, self.idpdy, self.turbulence.num)
            ...
            if self.runtype == BAROCLINIC:
                ...
                numpy.sqrt(self.momentum.ustar2_s.all_values, out=self.ustar_s.all_values)
                numpy.sqrt(self.momentum.ustar2_b.all_values, out=self.ustar_b.all_values)
                ...
                self.turbulence.advance(self.macrotimestep, self.ustar_s, self.ustar_b, self.z0s, self.domain.T.z0b, self.NN, self.momentum.SS)

                # Advect and diffuse tracers. Source terms are optionally handled too, as part of the diffusion update.
                self.tracers.advance(self.macrotimestep, self.momentum.uk, self.momentum.vk, self.momentum.ww, self.turbulence.nuh)
    ...
    def update_forcing(self, macro_active: bool, skip_2d_coriolis: bool=False, update_z0b: bool=False):
        ...
        if self.runtype == BAROCLINIC and macro_active:
            ...
            self.update_internal_pressure_gradient(self.buoy, self.momentum.SxB, self.momentum.SyB)
        ...
        self.momentum.update_depth_integrated_diagnostics(self.timestep, skip_coriolis=skip_2d_coriolis, update_z0b=update_z0b)
