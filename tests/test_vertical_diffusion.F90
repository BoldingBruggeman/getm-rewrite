...
!  Local constants
   integer, parameter :: imin=1, imax=200, jmin=1, jmax=50
...
!-----------------------------------------------------------------------------
   ...
   allocate(nuh(imin:imax,jmin:jmax,kmin-1:kmax))
   nuh = Av
   ...
   call vertical_diffusion%initialize(var,halo=halo)
   do n=1,Nmax
      t=t+dt
      write(*,*) n,' of ',Nmax
      call vertical_diffusion%calculate(timestep,cnpar,mask,dz,dz,avmol,nuh(:,:,1:),var)
      conservation=sum(var)
...
!-----------------------------------------------------------------------------
   ...
   subroutine field_manager_setup()
      ...
      call fm%register('nuh','m2/s','difusion',dimensions=(/id_dim_lon,id_dim_lat,id_dim_z/),no_default_dimensions=.true.,data3d=nuh(:,:,1:))
